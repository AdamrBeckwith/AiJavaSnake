...
class SpampedeData {
	...
	public void shrinkWalls() {
		...
		for (int row = 0; row < height; row++) {
			this.boardCells2D[row][wallsTot].becomeWall();
			
			this.boardCells2D[row][width - wallsTot - 1].becomeWall();
			
		}
		
		for (int column = 0; column < width; column++) {
			this.boardCells2D[wallsTot][column].becomeWall();
			
			this.boardCells2D[height - wallsTot -1 ][column].becomeWall();
		}
		wallsTot += 1; 
	}
	/** 
	...
	/* --------------------- */
	/**moveIntoSpam()
	 * Updates the snake to have the head now in the input cell, and stretch from the
	 * previous cell. The tail does not change position. 
	 * @param BoardCell cell
	 */
	public void stretchForward(BoardCell cell) {
	...
	/**shrinkTail() 
	 * shrinks the length of the tail of the Snake by one, assumes the tail exists. 
	 */
	public void shrinkTail() {
		BoardCell removedCell = this.snakeCells.remove();
		if(removedCell.isWall()) {
			removedCell.becomeWall();
		} else {
			removedCell.becomeOpen();
		}
	...
	public BoardCell getNorthNeighbor(BoardCell cell) {
		...
		return this.boardCells2D[CurrentRow - 1][CurrentCol ];  
	}
	...
	public BoardCell getEastNeighbor(BoardCell cell) {
		int CurrentCol = cell.getColumn();
		int CurrentRow = cell.getRow(); 
		return this.boardCells2D[CurrentRow][CurrentCol + 1]; 
	}
	...
	public BoardCell getNextCellInDir() {
		if(this.currentMode.equals(SnakeMode.GOING_NORTH)) {
			return this.getNorthNeighbor();
		}
		if(this.currentMode.equals(SnakeMode.GOING_SOUTH)) {
			return this.getSouthNeighbor();
		}
		if(this.currentMode.equals(SnakeMode.GOING_EAST)) {
	...
	/**flipHead 
	 * flips the snake around so it faces the opposite direction while retaining relationships 
	 * also does not update direction. 
	 */
	public void flipHead() {
		...
		for(int i= 0;i < this.snakeCells.size();i++) { 
			tempList.addFirst(this.snakeCells.get(i));
		}
		this.snakeCells = tempList;
		
		this.snakeCells.getLast().becomeHead();
	}

	/**findDirection
	 * determines the direction the snake should be heading in, and updates the SNAKEMODE
	 * to match  
	 */
    public void updateDirection() {
    	...
    	if(head.equals(getEastNeighbor(neck))) {
    		setDirectionEast();
    	}
    	if(head.equals(getWestNeighbor(neck))) {
    		setDirectionWest();
    	}
    	if(head.equals(getNorthNeighbor(neck))) {
    		setDirectionNorth();
    	}
    	if(head.equals(getSouthNeighbor(neck))) {
    		setDirectionSouth();
    	}
