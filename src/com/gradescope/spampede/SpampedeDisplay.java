...
public class SpampedeDisplay {
	...
	void updateGraphics() {
		...
		// Draw the board
		int height = this.theData.getNumRows();
		int width = this.theData.getNumColumns();
		
		for (int row = 0; row < height; row++) {
			for (int col = 0; col < width; col++) {
				BoardCell currentCell = this.theData.getCell(row, col);
				Color currentColor = currentCell.getCellColor();
				
				int y_CellCoord = currentCell.getRow() * Preferences.CELL_SIZE;
				int x_CellCoord = currentCell.getColumn() * Preferences.CELL_SIZE;
				
				drawSquare(x_CellCoord ,y_CellCoord ,currentColor);
			}
		}
	...
	public void drawSquare(int x, int y, Color cellColor) {
		int gameBoardShiftRight = (this.width - Preferences.NUM_CELLS_WIDE * Preferences.CELL_SIZE)/2; 
		int gameBoardShiftDown = (Preferences.TITLE_Y + Preferences.TITLE_FONT.getSize()/2);
		this.theScreen.setColor(cellColor);
		this.theScreen.fillRect(x + gameBoardShiftRight, 
				y + gameBoardShiftDown,
