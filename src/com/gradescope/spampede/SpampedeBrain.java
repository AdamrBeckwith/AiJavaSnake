...
public class SpampedeBrain extends SpampedeBrainParent {
	...
	public void cycle() {
		...
		if(this.cycleNum % Preferences.SHRINK_RATE == 0) {
			this.theData.shrinkWalls();
		}
	}
	...
	public void advanceTheSnake(BoardCell nextCell) {
		...
		if (nextCell.isWall() || nextCell.isBody()) {
			...
			this.theData.stretchForward(nextCell);
		} else {
			this.theData.stretchForward(nextCell);
			this.theData.shrinkTail();
	...
	public BoardCell getNextCellFromBFS() {
		...
		while(!(cellsToSearch.isEmpty())) {
			BoardCell searching = cellsToSearch.poll();
			BoardCell[] neighbors = this.theData.getNeighbors(searching);
			for(int i = 0; i < 4; i++) {
				BoardCell tempCell = neighbors[i];
				
				if(tempCell.getParent() == null && 
						(tempCell.isSpam() || tempCell.isOpen())) {
					tempCell.setParent(searching);
	...
	private BoardCell getFirstCellInPath(BoardCell start) {
		if(start.getParent().equals(this.theData.getSnakeHead())) {
			return start; 
		} else { 
	...
	public void reverseSnake() {
		this.theData.flipHead();
		this.theData.updateDirection();
	}
